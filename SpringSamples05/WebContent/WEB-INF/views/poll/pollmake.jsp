	...
	<td style="text-align: left;">
		<select name="syear">
			...
			for(int i = tyear; i< tyear+6; i++){
				%>
				<option <%=(tyear+"").equals(i+"")?"selected":"" %> value="<%=i %>"><%=i %></option>
				<%
			}
			%>
		</select>년
		<select name="smonth">
			...
			for(int i = 1; i<= 12; i++){
				%>
				<option <%=(tmonth+"").equals(i+"")?"selected":"" %> value="<%=i %>"><%=i %></option>
				<%
			}
			%>
		</select>월
		<select name="sday">
			...
			for(int i = 1; i<= cal.getActualMaximum(Calendar.DAY_OF_MONTH); i++){
				%>
				<option <%=(tday+"").equals(i+"")?"selected":"" %> value="<%=i %>"><%=i %></option>
				<%
			}
			%>
		</select>일
...
<tr>
	...
	<td style="text-align: left;">
		<select name="itemcount" onchange="pollchange(this)">
			...
			for(int i = 2; i<=10; i++){
				%>
				<option <%=(4+"").equals(i+"")?"selected":"" %> value="<%=i %>"><%=i %></option>
				<%
			}
			%>
		</select>
...
// 보기 갯수 갱신
function pollchange( obj ){	//  select obj 들어옴
